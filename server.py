...
def register():
    ...
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first() or User.query.filter_by(email=email).first():
            flash(translations[lang]['username'] + ' or ' + translations[lang]['email'] + ' already exists.', 'error')
...
def login():
    ...
    if request.method == 'POST':
        ...
        flash('Invalid ' + translations[lang]['username'] + ' or ' + translations[lang]['password'] + '.', 'error')
...
def add_to_cart(product_id):
    ...
    if 'user_id' not in session:
        flash('Please ' + translations[lang]['login'] + ' to add items to your ' + translations[lang]['cart'] + '.',
              'error')
        return redirect(url_for('login'))
    product = Product.query.get_or_404(product_id)
    ...
    if product.stock < quantity:
        flash(translations[lang]['insufficient_stock'].format(product.name_en if lang == 'en' else product.name_ru),
              'error')
        return redirect(url_for('product_detail', product_id=product_id))
...
def apply_promo():
    lang = session.get('lang', 'en')
    if 'user_id' not in session:
        flash('Please ' + translations[lang]['login'] + ' to apply promo codes.', 'error')
        return redirect(url_for('login'))
...
def remove_from_cart(item_id):
    lang = session.get('lang', 'en')
    if 'user_id' not in session:
        flash('Please ' + translations[lang]['login'] + ' to modify your ' + translations[lang]['cart'] + '.', 'error')
        return redirect(url_for('login'))
    ...
    db.session.commit()
    flash(translations[lang]['remove'] + ' from ' + translations[lang]['cart'] + '.', 'success')
    return redirect(url_for('cart'))
...
def place_order():
    lang = session.get('lang', 'en')
    if 'user_id' not in session:
        flash('Please ' + translations[lang]['login'] + ' to place an order.', 'error')
        return redirect(url_for('login'))
...
def orders():
    lang = session.get('lang', 'en')
    if 'user_id' not in session:
        flash('Please ' + translations[lang]['login'] + ' to view your ' + translations[lang]['orders'] + '.', 'error')
        return redirect(url_for('login'))
